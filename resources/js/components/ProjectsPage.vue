...
<div class="flex w-full h-full flex-wrap relative -mx-1 lg:-mx-4">
    ...
    <div :key="project.id" class="my-3 px-3 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3" v-for="project in this.getProjects">
            <router-link class="hover:no-underline" :to="'/project/'+project.name">
                <div class="bg-white min-h-full max-w-xs mx-auto shadow-lg border-b-4 border-indigo-500 rounded-2xl overflow-hidden  hover:shadow-2xl transition duration-500 transform hover:scale-105 cursor-pointer" >
                    ...
                    <div v-if="project.average_hours">
                        ...
                        <div class="py-2 ml-4 text-lg tracking-wide">Author:
                            <img v-if="project.author.logo" class="inline-block h-6 w-6 rounded-full ring-2 ring-white" :src="'/storage/images/profile/'+project.author.logo" alt="" >
                            <img v-if="!project.author.logo" class="inline-block h-6 w-6 rounded-full ring-2 ring-white" src="/storage/images/user.png" alt="" /><div class="inline-block">{{ project.author.name }}</div>
                        </div>
                </div>
            </router-link>
    </div>

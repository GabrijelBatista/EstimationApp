...
<div id="grid" class="flex w-full flex-wrap relative -mx-1 lg:-mx-4">
            ...
            <router-link class="hover:no-underline" :to="'/project/'+project.name">
                <div class="bg-white h-96 max-w-xs mx-auto shadow-lg border-b-4 border-cta-dark rounded-2xl overflow-hidden  hover:shadow-2xl transition duration-500 transform hover:scale-105 cursor-pointer" >
                    ...
                    <div v-if="project.average_hours">
                        ...
                        <p class="py-1 ml-4 text-lg text-black tracking-wide">Modules: {{ project.modules.length }}</p>
                        <p class="py-1 ml-4 text-lg text-black tracking-wide">Tasks: {{ project.tasks }}</p>
                        <p class="py-1 ml-4 text-lg text-black tracking-wide">Last update: {{ project.updated }}</p>
                    </div>
                        <div class="py-1 ml-4 text-lg tracking-wide text-black">
    ...
    <div v-if="modal" class="fixed top-0 left-0 right-0 bottom-0 w-full h-screen z-50 overflow-hidden bg-gray-darkest bg-opacity-75 flex flex-col items-center justify-center">
        <div class="w-full md:w-1/3 items-center p-6 space-x-6 bg-cta-light rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-500">
            <div class="font-extrabold text-black mx-auto text-2xl mb-4">CREATE PROJECT</div>
            <input v-model="name" type="text" required class="text-center appearance-none rounded-none relative block w-full mx-auto py-2 border border-cta-light placeholder-brand-dark text-black rounded-md focus:outline-none focus:ring-brand focus:border-brand focus:z-10 sm:text-sm" placeholder="Project Name" />
            <Multiselect
                v-model="assigned_to"
            ...
            <div class="mx-auto block">
                <div @click="modal=false" class="inline-block bg-brand hover:bg-brand-dark px-4 float-left mt-4 mx-auto py-2 text-white font-semibold rounded-lg hover:shadow-lg transition duration-3000 cursor-pointer">
                    <span>Cancel</span>
                </div>
                <div @click="add_project" class="inline-block bg-brand hover:bg-brand-dark px-4 float-right mt-4 mx-auto my-2 py-2 text-white font-semibold rounded-lg hover:shadow-lg transition duration-3000 cursor-pointer">
                    <span>Save</span>
                </div>
            </div>
        </div>

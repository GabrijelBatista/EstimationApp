    ...
    <Disclosure as="nav" class="bg-brand-dark" v-slot="{ open }">
        <div class=" mx-auto px-2 sm:px-6 lg:px-8">
            <div class="relative flex items-center justify-between h-16">
                <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
                    <!-- Mobile menu button-->
                    <DisclosureButton class="inline-flex items-center justify-center p-2 rounded-md bg-brand-light text-brand-dark hover:text-brand-dark hover:bg-brand focus:outline-none focus:ring-2 focus:ring-inset focus:ring-brand">
                ...
                <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                    <div class="flex-shrink-0 flex items-center">
                        <img class="block lg:hidden h-8 w-auto" src="/storage/images/estimator.png" alt="Estimator" />
                        <img class="hidden lg:block h-8 w-auto" src="/storage/images/estimator.png" alt="Estimator" />
                    </div>
                    <div class="hidden sm:block sm:ml-6">
                        <div v-if="this.getUser" class="flex space-x-4">
                            <router-link v-for="item in navigation" :key="item.name" :to="item.href" active-class="bg-brand text-cta-light" class="text-brand hover:no-underline bg-brand-light hover:bg-brand hover:text-brand-light px-3 py-2 rounded-md text-sm font-medium">{{ item.name }}</router-link>
                        </div>
                    </div>
                </div>
                <div v-if="this.getUser" class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    <Menu as="div" class="ml-3 relative">
                            <MenuButton active-class="bg-brand text-cta-light" class="text-brand hover:no-underline bg-brand-light hover:bg-brand hover:text-brand-light p-1 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand focus:ring-brand">
                        ...
                        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                            <MenuItems class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-brand text-brand-light ring-1 ring-black ring-opacity-5 focus:outline-none">
                                <MenuItem v-slot="{ active }">
                                    <router-link :to="'/profile/'+this.getUser.name" class="hover:text-brand-dark text-cta-light hover:no-underline w-full" :class="[active ? 'bg-brand-light' : '', 'block px-4 py-2 text-sm text-brand-dark']">My Profile</router-link>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                    <button @click="logout" class="hover:text-brand-dark text-cta-light w-full" :class="[active ? 'bg-brand-light' : '', 'block px-4 py-2 text-sm text-brand-dark']">Sign out</button>
                                </MenuItem>
                            </MenuItems>
                        </transition>
                    </Menu>
                ...
                <div v-if="!this.getUser" class="hidden sm:block sm:ml-6">
                    <div class="flex space-x-4">
                        <router-link to="/login" active-class="bg-brand text-cta-light" class="text-brand hover:no-underline bg-brand-light hover:bg-brand hover:text-brand-light px-3 py-2 rounded-md text-sm font-medium">Login</router-link>
                        <router-link to="/register" active-class="bg-brand text-cta-light" class="text-brand hover:no-underline bg-brand-light hover:bg-brand hover:text-brand-light px-3 py-2 rounded-md text-sm font-medium">Register</router-link>
                    </div>
                </div>
            </div>
        </div>
        ...
        <DisclosurePanel class="sm:hidden">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <router-link v-for="item in navigation" :key="item.name" :to="item.href" active-class="text-cta-light bg-brand radius-2xl" class="block px-3 py-2 hover:no-underline bg-brand-light rounded-md text-brand-dark font-medium">{{ item.name }}</router-link>
            </div>
        </DisclosurePanel>
    </Disclosure>
</template>
